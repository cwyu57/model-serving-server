...
class UsageUseCase:
    ...
    async def get_all_model_usages(self) -> list[ModelUsageOutput]:
        """
        Get detailed usage logs for all models with timestamps.
        Uses a single JOIN query to avoid N+1 problem.
        """
        models = await self.model_repository.get_all_with_usages()
    ...
    async def get_all_model_usage_counts(self) -> list[ModelUsageCountOutput]:
        """
